...
async function loadPieChart(dt) {
  try {
    ...
    showChart();
    const totalOrders = arr_status.reduce((sum, s) => sum + s.quantity, 0);
    $("#totalOrder").html(totalOrders);
  } catch (error) {
    console.error("Error loading pie chart:", error);
    if (error.status === 403) {
      showAlert("error", "Bạn không có quyền truy cập dữ liệu này. Vui lòng kiểm tra quyền hạn.");
    } else {
      showAlert("error", "Đã có lỗi xảy ra khi tải dữ liệu: " + (error.statusText || "Unknown error"));
    }
  }
}
$(document).ready(async function () {
  ...
  await loadPieChart();
  let countUser, totalRevenue, topProduct;
  try {
    ...
    showAlert("error", "Lỗi tải dữ liệu phân tích. Vui lòng kiểm tra quyền hạn của bạn.");
    return;
  }
  ...
  ${value.inventory} sản phẩm
    ...
    if (e == "inRange") {
      ...
      if (!val || !val.includes(" to ")) {
        showAlert("warning", "Vui lòng chọn đủ khoảng thời gian");
        return;
      }
    ...
    // Import functionality has been removed
    const totalImport = []; // Empty array since imports are no longer available
    let importValue = 0; // Set to 0 since imports are no longer available
