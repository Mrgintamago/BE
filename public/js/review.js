        ...
        },
        {
          ...
          render: function (row) {
            ...
            if (canEdit) {
              ...
              buttons.push('<button type="button" class="btn btn-danger btn-sm delete" data-id="' + (row.id || row._id) + '"><i class="fa fa-trash-alt"></i></button>');
            }
...
$(document).on("click", ".edit", function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#rating").val(review.rating || 5).trigger("change");
      $("#id").val(review._id);
    },
    error: function (error) {
      showAlert("error", "Không thể tải thông tin review");
...
$(document).on("click", ".delete", function () {
  ...
  if (confirm("Are you sure you want to delete this Review?")) {
    $.ajax({
      ...
      error: function (error) {
        showAlert("error", error.responseJSON?.message || "Xóa review thất bại");
      },
    });
  }
});
